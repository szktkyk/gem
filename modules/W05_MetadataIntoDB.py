...
cursor.execute(
    "Create table metadata221017 (getool Text, pmid Text, pubtitle Text, pubdate Text, organism_name Text, genesymbol Text,  editing_type Text, gene_counts Integer, biopro_id Text, RNA_seq Text, vector Text, cellline Text, tissue Text, Mutation_type Text)"
)

traffic = json.load(open("../csv_gitignore/20221014_ge_metadata.json"))
...
for row in traffic:
    ...
    cursor.execute("insert into metadata221017 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", keys)
    # print(f'{row["pubmed_id"]} data inserted Succefully')

rs = cursor.execute("select distinct organism_name from metadata221017")
