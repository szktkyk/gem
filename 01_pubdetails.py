...
import subprocess
import config
from modules import eutils, pubtator
import os
...
def main():
    ...
    """
    years_list = eutils.get_yearlist(1990)
    pmids_list = []
    for a_year in years_list:
        a_year = int(a_year)
        # id取得
        id_res = eutils.call_esearch(config.search_query, a_year)
    ...
    with open(
        f"./data/publication_details/{config.date}_pubdetails.csv",
        "w",
    ) as csvfile:
...
def get_pubdetails(pmids: list, max_len: int) -> list:
    ...
    list_of_chunked_pmids = pubtator.generate_chunked_id_list(pmids, max_len)
    pmids_metadata = []
