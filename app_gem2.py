...
import sqlite3
import pandas as pd
from modules import dash_data_gem2
...
title_html = html.Div(
    [
        ...
        html.P(
            "Search and retrieve metadata from the table/Figure/drop-down menu below",
        ),
    ],
    style={
        "width": "48%",
...
df2s.to_csv("./csv_gitignore/tax_list_fig2s.csv",columns=['species'],index=True)

list_speciesfig_left = dash_data_gem2.data_for_speciesfig_left(DATABASE)
df3 = pd.DataFrame(
...
df3s.to_csv("./csv_gitignore/tax_list_fig3s.csv",columns=['species'],index=True)

# df4 = pd.read_csv(f"{date}_metadata.csv")
con = sqlite3.connect(DATABASE)
df4 = pd.read_sql_query(f'select * from {config.sql_table_name}',con)
...
allgetools = df4['Genome Editing Tool'].unique().tolist()
...
alldata_html = html.Div(
    [
        html.Button(
            "All DATA (takes a while to load)",
            style={"fontSize": 20, "textAlign": "left"},
...
getools_html = html.Div(
    [
        ...
        html.P(
            "Select a combination of genome editing tools from the dropdown options",
        ),
    ],
    style={
        # "width": "49%",
...
searchbypub_html = html.Div(
    [
        ...
        html.P("Select the area on the figure below to retrive metadata"),
        dcc.Graph(
            id="fig1",
            figure=px.bar(
                df,
...
taxonomy_category_html = html.Div(
    [
        ...
        html.P(
            "Select a Taxonomy category from the dropdown options",
        ),
    ],
    style={
        # "width": "49%",
...
species_html = html.Div(
    [
        ...
        html.P(
            "Select a species from the dropdown options or select the area on the figure below to retrive metadata",
        ),
    ],
    style={
        # "width": "49%",
...
app.layout = html.Div(
    children=[
        ...
        dcc.Dropdown(
            ...
            options=[{"label":st,"value":st} for st in allgetools],
...
def show_hover_data(selectedData):
    if selectedData is None:
        ...
        # print(selectedData)
        df_selected_fig1 = dash_data_gem2.parse_callback_json_fig1(selectedData, DATABASE)
        table_selected_fig1 = dash_data_gem2.selected_table(df_selected_fig1)
...
def CreateTableFig4(selectedData):
    if selectedData is None:
        ...
        # print(selectedData)
        df_selected_fig4 = dash_data_gem2.parse_callback_json_fig4(selectedData, DATABASE)
        table_selected_fig4 = dash_data_gem2.selected_table(df_selected_fig4)
...
def CreateTableFig2Left(selectedData):
    if selectedData:
        df_selected_fig2_left = dash_data_gem2.parse_callback_json_fig2_left(selectedData, DATABASE)
        table_selected_fig2_left = dash_data_gem2.selected_table(df_selected_fig2_left)
...
def CreateTableFig2right(selectedData):
    if selectedData:
        df_selected_fig2_right = dash_data_gem2.parse_callback_json_fig2_right(selectedData, DATABASE)
        table_selected_fig2_right = dash_data_gem2.selected_table(df_selected_fig2_right)
...
def display_table2(value):
    if value:
        datalist = dash_data_gem2.data_for_taxonomy_category_fig(DATABASE,value)
        df6 = pd.DataFrame(
            data=datalist,
        ...
        df_taxonomy_cateogry = dash_data_gem2.parse_callback_json_taxonomy_category(value,DATABASE)
        table_filter_dropdown2 = dash_data_gem2.taxonomy_category_selected_table(df_taxonomy_cateogry)
...
def display_table(value):
    if value:
        datalist = dash_data_gem2.data_for_species_fig(DATABASE,value)
        df05 = pd.DataFrame(
            data=datalist,
            columns=["genome editing tools", "species", "the number of genes studied"],
        )
        ...
        df_species = dash_data_gem2.parse_callback_json_species(value,DATABASE)
        table_filter_dropdown = dash_data_gem2.species_selected_table(df_species)
...
def display_table_getools(value):
    if value:
        df_getools = dash_data_gem2.parse_callback_json_getools(value,DATABASE)
        table_filter_dropdown_getools = dash_data_gem2.getools_selected_table(df_getools)
